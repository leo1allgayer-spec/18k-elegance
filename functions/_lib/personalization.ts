...
export async function uploadProductImage(request: Request, env: Env, productId: number): Promise<Response> {
  ...
  const previous = append ? null : await env.DB.prepare("SELECT url FROM product_images WHERE product_id=? ORDER BY sort_order,id LIMIT 1").bind(productId).first<{ url: string }>();
  ...
  if (!append) await env.DB.prepare("DELETE FROM product_images WHERE product_id=?").bind(productId).run();
  const order = append ? Number((await env.DB.prepare("SELECT COALESCE(MAX(sort_order),-1)+1 AS value FROM product_images WHERE product_id=?").bind(productId).first<{ value: number }>())?.value || 0) : 0;
  await env.DB.prepare("INSERT INTO product_images(product_id,url,alt_text,sort_order) VALUES(?,?,?,?)").bind(productId, url, product.name, order).run();
